...
export default function AdminDashboard() {
  ...
  const exportToCSV = () => {
    // ... (igual que antes)
    if (votes.length === 0) return alert('No hay votos')
    const headers = ['Ciudad', 'Lugar', 'Cédula', 'Fecha']
    const rows = votes.map(v => [v.city, v.place, v.cedula || 'N/A', new Date(v.created_at).toLocaleString('es-CO')])
  ...
  votes.forEach(v => {
    ...
    if (v.cedula) uniqueVoters.add(v.cedula)
  })

  const totalVotes = votes.length
  const uniqueCount = uniqueVoters.size

  const cityChartData = Object.entries(cityTotals).map(([city, votos]) => ({ ciudad: city, votos }))
    ...
    return (
      <div className="max-w-md mx-auto mt-16 px-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 text-center">
          ...
          <h2 className="text-3xl font-bold">Supervisor</h2>
          <input
            type="password"
            value={pin}
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      <div className="flex justify-between items-center mb-8">
        <div>
          <div className="text-cyan-400 text-sm tracking-[3px]">SUPERVISOR • AUTO-REFRESH 15s</div>
          <div className="text-5xl font-bold tracking-tighter">Panel de Supervisión</div>
        </div>
        <button onClick={exportToCSV} className="flex items-center gap-2 px-6 py-3 bg-zinc-900 rounded-2xl border border-zinc-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
          ...
          <div className="text-6xl font-mono font-bold mt-1 text-emerald-400">{uniqueCount}</div>
        </div>
      </div>
          ...
          {Object.keys(PLACES).map(city => {
            ...
            return (
              <div key={city} className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
                ...
                )) : <p className="text-zinc-400 text-sm">Sin votos aún</p>}
              </div>
            )
          })}
        </div>
      ...
      {/* Tabla */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
        <div className="font-semibold mb-4">Últimos 80 votos</div>
            ...
            <tbody>
              {votes.slice(0, 80).map((v, i) => (
                <tr key={i} className="border-b border-zinc-800">
                  ...
                  <td>{v.place}</td>
                  <td className="text-xs text-zinc-400">{v.cedula || '—'}</td>
