...
export default function AdminDashboard() {
  ...
  const loadVotes = async () => {
    ...
    const { data, error: fetchError } = await supabase
      .from('votes').select('*').order('created_at', { ascending: false })
    if (fetchError) {
      console.error('Error cargando votos:', fetchError)
      setError(`${fetchError.message} (${fetchError.code})`)
    }
    if (data) { setVotes(data); setLastUpdated(new Date()) }
    setLoading(false)
  }
  ...
  const handleLogin = () => {
    if (pin.trim() === ADMIN_PIN) { setIsAuthed(true) }
    ...
    <div className="max-w-6xl mx-auto px-6 py-10">
      ...
      {error && (
        <div className="mb-6 border border-red-900 rounded-lg px-4 py-3 text-red-400 text-xs">
          ...
          <br />
          <span className="text-red-600">Revisa las políticas RLS en Supabase → tabla "votes"</span>
        </div>
      )}
